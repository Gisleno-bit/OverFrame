...
The Kestrel gate includes all 22 action IDs and 23 current variants, both facings, exact designated pieces, all real active/late ticks, charge, jab chaining and hitlag. Neutral special requires its actual projectile emission and any fighter hitbox that exists for the case. The deliberate Kestrel-only removal is recorded in the gameplay review; Boulder/Viper mixed events still require the supplement under the animation contract. Ground support requires both foot bounds and their support-plane measurements. Preserve existing simulation, damage, frame data, hitboxes, capsules, model scale and bone lengths; report unreachable geometry.
...
When comparing different local checkouts of the same Cargo package, give the baseline and candidate independent target directories. A Windows audit found that the shared target directory reused a corrected library in a baseline test and falsely passed it. That run was discarded; both sides rebuilt from independent caches. Preserve the identical portable test hash, compiler exit and complete before/after traces.
