...
An exception names an event to review; it does not erase another real runtime hitbox. The gameplay correction reviewed in [4ddb103](../../reviews/4ddb10315b936f8b70e5957a5adcd986e8039e9c.md) deliberately removes only Kestrel's zero-damage SpecialN fighter hitbox. The old tick-8/state-frame-9 event is historical evidence, not a remaining visual requirement. Its actual projectile emission is still after_step tick 0/state_frame 1 in the declared fixture. Review the hand against the actual emission origin; it does not have to follow the projectile in flight. Boulder and Viper retain their real SpecialN fighter hitboxes, including zero-damage collision behavior, and require their contact evidence.
...
For an action with a real fighter hitbox, all actual clean/late contact remains mandatory. The capture index declares `contact_expected[].fighter_hitbox` from the move table. Only an explicit boolean `false` permits omitting the fighter-hitbox supplement; missing fields retain the older requirement and malformed values fail validation. The reviewer also checks actual replay samples for absence of the hitbox and presence of the projectile. Kestrel's removal does not authorize blanket exemptions for projectile actions or zero-damage hitboxes.
